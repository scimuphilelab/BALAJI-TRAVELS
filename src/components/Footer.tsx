...
const bankDetails = [
  ...
  { label: "Account Number", value: "732701010050211" },
  { label: "IFSC Code", value: "UBIN0573272" },
  { label: "Account Name", value: "Balaji Travels" },
  { label: "Branch Address", value: "Kunjabanagarh, Daspalla" },
  { label: "UPI / PhonePe / GPay", value: "8658169299@ibl" },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-white/50 text-sm leading-relaxed mb-6 max-w-xs">
              Bhubaneswar's most trusted car rental service since 2016. Your
              Destination is our Priority — always.
            </p>
          ...
          {/* Bank Details */}
          <nav aria-label="Footer bank details">
            <h3 className="text-white font-semibold text-sm uppercase tracking-widest mb-5">
              Bank Details
            </h3>
            <ul className="space-y-2">
              {bankDetails.map((item) => (
                <li key={item.label} className="flex items-start gap-2 group">
                  <span
                    className="w-1 h-1 rounded-full bg-[#C9A84C] mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                    aria-hidden="true"
                  />
                  <span className="text-xs text-gray-300 leading-snug whitespace-nowrap">
          ...
          <div>
            <img src="/payment-qr.jpg" alt="Logo" className="w-50 ml-4 -mt-4 " />
          </div>
            ...
            <ul className="space-y-2">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-white/50 text-xs hover:text-[#C9A84C] transition-colors duration-200 flex items-center gap-2 group"
                  >
                    <span
